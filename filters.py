...
def enhanced_image(image):
    ...
    if color:
        enhanced_image = enhanced_image_color_type2(image)
    if sharpness:
        enhanced_image = enhanced_image_sharpness(image)
    if noise:
        enhanced_image = enhanced_image_decrease_noise(image)
...
def enhanced_image_color_type1(original_image):
...
def enhanced_image_color_type2(image):
...
def transform(original_img, width, height):
    original_img = enhanced_image(original_img)
    method = cv2.INTER_CUBIC
    if width > crop_max or height > crop_max:
        method = cv2.INTER_AREA
        if width > crop_max and width > height:
            width = crop_max
        if height > crop_max and width < height:
            height = crop_max

    original_img = image_resize(original_img, width, height, method, False)
