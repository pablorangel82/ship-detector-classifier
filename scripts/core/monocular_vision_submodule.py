...
class MonocularVision:
    ...
    def monocular_vision_detection_method_2(camera, real_height, detected_bbox):
        ...
        fator = (x_center_pixel - x_center_frame) / (x_center_frame)
    ...
    def monocular_vision_detection_method_3(camera, detected_bbox):
        # bearing estimation with arctan relations (more accurate then the linear interpolation (method 2))
        pixel_width = detected_bbox[2]
        x_center_pixel = detected_bbox[0] + (pixel_width / 2.0)
        x_center_frame = camera.sensor_width_resolution / 2.0
        dx = x_center_pixel - x_center_frame
        diff_degrees = math.degrees(math.atan(dx / camera.fx))
        bearing = (camera.bearing + diff_degrees) % 360

        # distance estimation with tilt angle from camera (compare with height based estimator)
        bottom_y_px = detected_bbox[1] + detected_bbox[3]
        dy = bottom_y_px - camera.sensor_height_resolution / 2.0
        delta_angle_rad = math.atan(dy / camera.fy)
