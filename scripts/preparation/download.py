    ...
    while attempts <= attempts_limit:
        try:
            ...
            if attempts > attempts_limit:
                logging.debug('Cannot able to download content.')
                return None
            logging.debug("Connection error. Trying again: " + str(attempts))
            attempts = attempts +1
    response = con.read()
    return response
...
def save_data(eni, category, outFolder):
    ...
    if json_values is not None:
        try:
            ...
            logging.debug('Data not available: ' + str (eni))
            return False
        ...
        if (len(photos) +2) <= number_of_files:
            logging.debug('Data already downloaded: ' + str(imo))
            return True
        else:
            logging.debug('Expected: ' + str(len(photos) +2) + '. Found: ' + str(number_of_files))
        past = []
        logging.debug(url)
        ...
        for h in photos:
            ...
            try:
                ...
                logging.debug("Former informations are not available. ")
...
def download (start_again, auto_resize, all_categories):
    global resize, categories
    ...
    if start_again:
        ...
        if os.path.exists(DatasetConfig.SRC_FOLDER):
    ...
    if resize == True:
        logging.info('Auto resize enabled to: ' + str(DatasetConfig.target_img_width) + ' x ' + str(DatasetConfig.target_img_width))
    else:
        logging.info('Auto resize is disabled. You should read copyright policies of shipspotting: https://www.shipspotting.com/support/68')
    ...
    for i in range (len(categories)):
        ...
        categories_by_name[categories[i].source_key] = i
    ...
    for index, eachLine in enumerate(downloadContent):
        ...
        cat = cat.replace('/', " ")
        temp.append (cat)
        index = int (categories_by_name[cat])
